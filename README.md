...
1. `_install.bat` をダブルクリックして実行してください。
2. 完了画面が表示されたらウィンドウを閉じてください。

構築される環境:
- Python仮想環境 (`venv`)
- 依存ライブラリ (`requirements.txt`)
- VOICEVOX Core (`voicevox_core/` ディレクトリ配下)

### 確認

インストールが正しく完了したか確認するために、以下のコマンドを実行してください。

```cmd
venv\Scripts\python.exe test_modules.py
```

全ての項目が `[OK]` と表示されれば準備完了です。
