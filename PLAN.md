...
- [x] **LLMクライアントモジュール** (`src/llm_client.py`)
    ...
    - **Ollama自動起動ロジック**の実装
- [x] **VOICEVOXアダプタ** (`src/tts_engine.py`)
    - `voicevox_core` (0.15.x) を初期化し、テキストから `AudioQuery` を生成する
    - 変調なしで音声合成(WAV出力)ができるまでのパイプライン構築

### Phase 2: 感情物理演算とアライメントの実装
トークンごとの相対変位と減衰（Decay）計算を実装し、自然な揺らぎを生成する。

- [x] **感情動態モデル** (`src/emotion_dynamics.py`) (旧: physics.py)
...
└── src/
    ...
    ├── emotion_dynamics.py # 感情動態モデル (減衰・変位)
